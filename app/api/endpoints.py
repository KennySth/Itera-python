...
async def run_scraper_task(query: str) -> None:
    ...
    results = await asyncio.gather(
        ct_scraper.scrape(query),
        li_scraper.scrape(query),
...
async def get_offers(limit: int = 10, skip: int = 0) -> List[JobOffer]:
    """
...
@router.get("/audit/scraping", response_model=List[ScrapingAudit], tags=["Audit (RF-09)"])
async def get_scraping_audit(limit: int = 20) -> List[ScrapingAudit]:
    ...
    """
    database = get_database()
    cursor = database["auditoria_scraping"].find().sort("fecha_ejecucion", -1).limit(limit)
...
async def record_telemetry(event: ExplorationTelemetry) -> Dict[str, str]:
    ...
    """
    database = get_database()
    await database["telemetria_exploracion"].insert_one(event.model_dump(by_alias=True, exclude_none=True))
...
async def record_feedback(feedback: RecommendationFeedback) -> Dict[str, str]:
    ...
    """
    database = get_database()
    await database["feedback_recomendaciones"].insert_one(feedback.model_dump(by_alias=True, exclude_none=True))
    return {"status": "recorded"}
